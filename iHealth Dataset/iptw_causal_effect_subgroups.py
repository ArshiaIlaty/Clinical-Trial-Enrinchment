...
def subgroup_iter(df, col):
    for val in sorted(df[col].dropna().unique()):
        yield val, df[df[col] == val]

def run_iptw(df, outcome_col, covariates, label):
    if df['TREATED'].nunique() < 2 or df[outcome_col].isnull().all():
        print(f"[SKIP] {label}: Not enough data for IPTW analysis.")
        return None
    X = df[covariates].copy()
    X = pd.get_dummies(X, columns=[c for c in X.columns if X[c].dtype == 'object'], drop_first=True)
    scaler = StandardScaler()
    for col in ['BASELINE_BP','BMI']:
        if col in X.columns:
            X[[col]] = scaler.fit_transform(X[[col]])
    ps_model = LogisticRegression(max_iter=1000)
    ps_model.fit(X, df['TREATED'])
    df['propensity'] = ps_model.predict_proba(X)[:,1]
    df['weight'] = np.where(df['TREATED']==1, 1/df['propensity'], 1/(1-df['propensity']))
    X_reg = sm.add_constant(df['TREATED'])
    wls_model = sm.WLS(df[outcome_col], X_reg, weights=df['weight'])
    results = wls_model.fit()
    print(f'\n=== IPTW Causal Effect in {label} ===')
    print(results.summary())
    print(f'Estimated causal effect (ATE) of high adherence (Q5 vs Q1) on {outcome_col}: {results.params["TREATED"]:.2f}')
    return results

# Main analysis for each subgroup
groupings = [
    ('CALCULATED_AGE_GROUP', 'Age Group'),
    ('GENDER_AT_BIRTH', 'Gender'),
    ('BMI_CATEGORY', 'BMI Category'),
    ('ASSIGNED_TREATMENT', 'Treatment Type')
]

covariates = ['BASELINE_BP','BMI','GENDER_AT_BIRTH','ASSIGNED_TREATMENT']

for col, label in groupings:
    print(f'\n===== IPTW Causal Effect by {label} =====')
    for val, subdf in subgroup_iter(full, col):
        print(f'\n--- {label}: {val} ---')
        run_iptw(subdf, 'BP_CHANGE', covariates, f'{label}: {val}')
        if 'A1C_CHANGE' in subdf.columns:
            run_iptw(subdf.dropna(subset=['A1C_CHANGE']), 'A1C_CHANGE', covariates, f'{label}: {val}')
        if 'WEIGHT_CHANGE' in subdf.columns:
            run_iptw(subdf.dropna(subset=['WEIGHT_CHANGE']), 'WEIGHT_CHANGE', covariates, f'{label}: {val}') 